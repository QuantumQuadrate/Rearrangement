  ...
  */
  ...
  for (size_t r = 0; r < costRows; ++r) {
    for (size_t c = 0; c < costCols; ++c) {
      cost.push_back(std::pow(std::pow(std::abs(occupiedCoords[c].x - desiredSites[r].x), 1.001f)
                                  + std::pow(std::abs(occupiedCoords[c].y - desiredSites[r].y), 1.001f), 1.01f));
    }
  }
...
void Rearranger::step4() {
  ...
  */
  ...
  while (proceed) {
    ...
    if (find_zero(&row, &col)) {    //An uncovered zero is found
      primedColInRow[row] = col;
      if (starredColInRow[row] == -1) {    // No starred zeros in the row
        step5(row, col);
        proceed = false;

      } else {    //A starred zero was found in the row
        isCoveredCol[starredColInRow[row]] = false;
        isCoveredRow[row] = true;
      }
    } else {    //No uncovered zeros were found
      step6();
      proceed = false;
...
std::string Rearranger::orderMovesCorner() {
  ...
  */
  ...
  for (unsigned int badIndex = 0; badIndex < badSites.size(); ++badIndex) { //Iterate over each coordinate we want empty
    wasOccupied = false;
    stayedStill = true;;
    for (unsigned int occIndex = 0; occIndex < occupiedCoords.size();
         ++occIndex) { //Determine if that site was occupied
    ...
    if (wasOccupied) {
      for (unsigned int moveIndex = 0; moveIndex < moves.size();
           ++moveIndex) { //Determine if the instructions so far moved it
      ...
      if (stayedStill) { //If it stayed still we add it as a corner move to the end of the normal list
        cornerMoveString +=
            std::to_string(badSites[badIndex].y) + ">" + std::to_string(badSites[badIndex].x) + ">-1>-1>";
        numCornerMoves += 1;
      }
    }
  }
